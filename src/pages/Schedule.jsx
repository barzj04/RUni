...
export default function Schedule({userId, displayName}) {
    ...
    const weekLabel = `${weekDates[0].toLocaleDateString('en-MY', { day: 'numeric', month: 'short' })} to ${weekDates[6].toLocaleDateString('en-MY', { day: 'numeric', month: 'short', year: 'numeric' })}`
    const prevKey = toKey(new Date(weekStart.getFullYear(), weekStart.getMonth(), weekStart.getDate() - 7))
    const prevWeek = mySchedule[prevKey]
    const prevHasData = prevWeek && DAYS.some(d => prevWeek[d]?.Lunch || prevWeek[d]?.Dinner)
    function copyLastWeek() {
      setMySchedule(prev => ({ ...prev, [weekKey]: JSON.parse(JSON.stringify(prev[prevKey] || {})) }))
    }
      ...
      <div className="bg-white rounded-xl shadow p-4 mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="w-full flex items-center justify-center gap-3">
          ...
          <button
            ...
            className="bg-rose-100 text-rose-500 hover:bg-rose-200 px-3 py-1 rounded-lg text-sm disabled:opacity-40"
          >
            Same as last week
          </button>
          {weekKey !== thisWeekKey && (
